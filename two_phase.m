...
% mpfad--> método linear dos volumes finitos baseado no Gao e Wu 2010
% tpfa ---> método Linear dos volumes finito 
% nlfvLPEW --> método não linear dos volumes finitos baseado no artigo  (Sheng e Yuan, 2011)
% nlfvDMPSY --> método não linear que preserva DMP baseado no artigo (Gao e Wu, 2013) e (Sheng e Yuan, 20...)
% lfvLPEW --> método linear based no método não linear usando LPEW;
% lfvHP --> método linear baseado no método não linear usando pontos
% harmônicos 
pmetodo='mpfad';
%pmetodo='nlfvLPEW';
...
auxflag=202;
%% adequação das permeabilidades segundo o bechmark
...
%while t_old<totaltime(2)
    ...
    %% quando a velocidade é unitario
